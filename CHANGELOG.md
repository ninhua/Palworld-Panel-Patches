...
- 将索引字段升级为可组合的模板筛选器：支持“索引文件、模板分类、综合分级、用途分类、分类标签、毕业用途、模板状态”七个维度，多项条件按同时满足组合。
- 新玩家礼包页与玩家中心模板发放区共用 `PalTemplateFilterBar` 和同一匹配函数；卡片继续显示帕鲁图片，并显示分级、用途、标签、毕业用途、毕业/过渡状态等索引信息。
- 搜索范围扩展到中文名、英文名、PalID、模板名、分类、用途、毕业用途、词条中文和夜间工作方式。

### Maintenance

- 功能继续合并进未发布的 `0038-add-starter-gift-debug-console-and-rich-template-indexes.patch`，没有新增补丁编号；补丁链仍为 `0001`–`0038`。
- `PalWorkspace.tsx` 的 old-side preimage 不变，独立官方夹具继续验证全部 hunk；新增组件也加入 `0038` changed-file allowlist 和回归标记。

### Validation

- 验证无索引模板可按“未命中索引”筛选，毕业帕鲁、当前毕业用途、低阶与过渡、普通模板可按状态筛选。
- 验证分类筛选和文本搜索在礼包配置与玩家中心模板发放两个入口使用同一数据源和相同逻辑。
- `0038` SHA-256、累计补丁应用、Go 回归、仓库元数据和稳定 Release 布局门禁通过。
...
- 新增 `0034-add-pal-inventory-advanced-filters.patch`，为现有帕鲁仓库增加多条件筛选、质量排序和终端格位显示。
- 后端支持最低等级、最低星级、最低平均 IV、性别、位置类型和多个被动词条的交集筛选，并支持等级、平均 IV、星级和名称排序。
- `/api/pals` 现在返回原索引中已有的性别、Rank、三项 IV、平均 IV、浓缩星级、槽位、位置类型、远征状态和终端位置。
- 帕鲁终端位置按 30 格/页、6 列换算为“第 N 页 · 第 N 行第 N 列”；队伍、据点工作位和远征状态使用对应中文位置。
- 前端增加高级筛选面板、星级/个体值列和终端位置列，移动端卡片同步展示。

### Validation

- 新增补丁文件集合、SHA-256、功能注册、筛选标记、终端格位公式和 Go 格式回归。
- stable patch 候选保持 `0.8.16`。
...
- 新增 `0032-read-starter-gift-template-indexes.patch`，扫描 PalDefender `Templates` 目录中的模板索引 JSON。
- 支持用户提供的“模板中英文对照索引”结构：通过 `文件名` / `模板名` 关联模板，读取 `分类`、`中文名`、`英文名` 和可选 `PalID`；模板自身 JSON 的 `PalID` 仍是头像与身份的唯一来源。
- 支持多个索引文件同时存在。每个有效索引都成为独立筛选项，因此“常用毕业帕鲁清单”等子集索引可直接用于批量选择。
- 帕鲁模板按“分类 → 帕鲁名 → 模板文件名”稳定排序，并增加索引文件、分类、中文名、英文名、PalID 和文件名联合搜索。
- 未检测到索引或模板未命中任何索引时，界面继续显示模板文件名，不使用文件名推断 PalID，也不伪造中文名。
- 索引文件本身不会出现在可发放模板列表中。

### Validation

- 增加双索引、中文字段、索引成员关系、索引文件排除和无索引文件名回退的 Go 回归。
- 增加 `0031 → 0032` 累计应用、OpenAPI 字段、前端索引/分类筛选和 SHA-256 检查。
- stable patch 候选保持 `0.8.16`。
...
- 新增 `0029-fix-starter-gift-selection-scroll.patch`，移除礼包物品和帕鲁模板列表的“已选项优先”动态排序。
- 修复点击一个物品后，该条目立即移动到列表顶部并带动内部滚动容器回弹的问题。
- 普通列表现在始终按名称稳定排序；“只看已选”仍正常过滤，数量编辑、批量选择和保存逻辑不变。

### Validation

- 新增累计补丁应用和静态排序回归，确保 `0024 → 0025 → 0026 → 0029` 可直接应用。
- stable patch 候选保持 `0.8.15`。
...
- 新增 `0027-add-unattended-inventory-delta.patch` 与 stable required feature `unattended-inventory-delta`。
- 复用 15 秒玩家在线采样；当前服务器世界无人在线时，从全服只读库存索引建立基线并计算物品正向净变化。
- 无人时段至少持续 5 分钟才保留；在线状态或索引采样中断超过 60 秒时重建基线，避免停机或网络故障形成虚假时长和差值。
- 状态、基线和最近一次完成结果按 WorldID 隔离；导入存档只用于浏览，不参与实时统计。
- 库存管理页面增加“无人时段库存净增加”卡片，展示状态、时长、有效门槛、世界、索引新鲜度和前八项净增加物品，并明确该指标不等同于生产量。

### Validation

- 新增状态机、世界隔离、容器去重、短时段丢弃、正向净变化和采样中断重置 Go 回归。
- 新增补丁文件白名单、SHA-256、累计基线应用、monitor 接入和前端状态卡标记回归。
- stable patch 版本升级为 `0.8.14`，预期 Release 标签为 `uitok-stable-v1.3.0-p0.8.14`。

## v0.12.28

### Changed

- 新增 `0026-redesign-starter-gift-ui.patch`，修复 `/starter-gift` 已注册路由但未加入 Sidebar 显式 `routeIDs`，导致左侧“玩家与世界”没有可点击入口的问题。
- 新玩家礼包页面改为状态摘要、紧凑发放策略、礼包物品/帕鲁模板/发放记录三个 Tab 和底部固定保存操作栏。
- 物品目录复用现有本地化 WebP 图标，缺图时显示占位图标；选中行直接提供数量减一、输入、加一和移除操作。
- 物品分类拆分为帕鲁球、工具、近战武器、枪械与弓、弹药与投掷物、防具与饰品、食物与农作物、药品、基础材料、高级材料、鞍具与关键物品、图纸与技能果实及其他。
- PalDefender 模板从模板文件名解析帕鲁 CharacterID，复用现有 `PalIcon` 显示头像，并保留原始 JSON 文件名。
- 物品和模板增加“只看已选”、当前结果全选/取消和选中项优先排序；发放记录增加状态统计和明确的状态徽标。

### Validation

- 新增 `test-starter-gift-ui-redesign.sh`，验证 0026 仅修改 Sidebar 与礼包页面、补丁 SHA-256、0024→0025→0026 累计应用、侧栏入口、图标、分类、Tab 和固定保存栏标记。
- stable patch 版本升级为 `0.8.13`，预期 Release 标签为 `uitok-stable-v1.3.0-p0.8.13`。
...
- 新增 `0025-fix-starter-gift-ui-and-save-scope.patch`，按 `GameUserSettings.ini` 的 `DedicatedServerName` 解析活动 WorldID，并将玩家在线时长、上线/下线事件、礼包配置、已见玩家、冻结计划和批次进度全部按世界隔离。
- 修复新建存档后继承旧档在线历史和礼包状态的问题；多个世界存在时不迁移旧全局历史，唯一世界升级时才执行兼容迁移。
- 修复礼包配置已启用时，新世界首次在线玩家被初始化流程标记为老玩家而不发放的问题。
- 新世界继承最新礼包配置作为初始模板；保存后形成独立世界配置，不会覆盖其他已初始化世界。
- 玩家中心增加“初始礼包”入口；物品目录改为始终可见的分类、搜索、点击多选和当前筛选全选/取消，不再要求手工输入 ItemID。
- 物品、帕鲁模板、已选物品和发放记录使用固定高度内部滚动区域，避免大量条目无限拉伸页面。
- 发放 worker 按世界分别串行，切换世界时不会因另一世界的任务占用全局 worker。
- 修正文档中已发布工作区状态和 blocked migration Issue/Draft PR 行为描述。

### Validation

- 新增活动 `DedicatedServerName` 优先解析、时间戳回退、跨世界在线历史隔离、唯一世界旧状态迁移、跨世界配置隔离和新世界首位玩家发放测试。
- 新增 0025 补丁文件白名单、SHA-256、作用域标记、前端入口、多选和滚动布局回归。
- stable patch 版本升级为 `0.8.12`，预期 Release 标签为 `uitok-stable-v1.3.0-p0.8.12`。
