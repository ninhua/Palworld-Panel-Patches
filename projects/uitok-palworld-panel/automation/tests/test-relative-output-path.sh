...
config="${repo_root}/projects/uitok-palworld-panel/automation/config.json"
active_track_rel="$(python3 - "${config}" <<'PY_TRACK'
from pathlib import Path
import json, sys
data = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
print(data["bootstrap_source_track"])
PY_TRACK
)"
active_track="${repo_root}/${active_track_rel}"
build_script="${active_track}/build/build-palpanel.sh"
python3 - "${active_track}/track.json" <<'PY_ASSERT'
from pathlib import Path
import json, sys
data = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
assert data.get("target_version") == "v1.3.0", data
assert data.get("status") == "candidate", data
assert data.get("source_mode") == "self-contained", data
assert not data.get("inherits"), data
PY_ASSERT
[[ -x "${build_script}" ]] || { echo "active track build script missing: ${build_script}" >&2; exit 1; }
tmp="$(mktemp -d)"
