...
usage() {
    echo "用法：$0 <palpanel二进制> <目标版本> <期望commit> <期望补丁版本> <features-json>" >&2
    exit 2
}
...
target_version="$2"
expected_commit="$3"
expected_patch_version="$4"
features_json="$(realpath "$5")"
...
[[ -s "${features_json}" ]] || {
    echo "缺少 feature 文件：${features_json}" >&2
    exit 1
}
...
data = json.loads(Path(response).read_text(encoding="utf-8"))
required = set(json.loads(Path(features_path).read_text(encoding="utf-8")))
...
features = set(payload["patch"].get("features", []))
assert required.issubset(features), (required, features)
