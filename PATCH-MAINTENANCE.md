...
同一 feature 的补丁尚未进入 stable Release 时，后续 bugfix、性能修正或测试修正必须重新生成并替换该 feature 的最新补丁，不得仅为 hotfix 连续增加新的编号。

例如 `0038` 与紧随其后的索引扫描修正属于同一 `new-player-starter-gift` 功能域，最终应保留一个合并后的 `0038`，而不是继续保留 `0039`。

允许新增补丁编号的条件是：

- 前一补丁已经进入已发布且不可变的 stable Release；或
- 新变更属于独立 feature，具有不同的生命周期、依赖和回滚边界。
...
目录中的旧补丁文件必须实际删除，不能只从 catalog 或 SHA-256 清单中移除。
...
## 4. 同一文件被后续补丁修改时必须重新变基

若补丁修改的文件已经被上游版本或前序补丁改变，必须重新生成该补丁的对应 file section。禁止仅修改 hunk 行号、降低上下文或依赖模糊匹配。

对于同一文件的多个 hunk，禁止从删除中间内容的拼接文件生成 diff。即使每个局部上下文都来自真实源码，不连续的伪造行距也会让前一个 hunk 的 offset 影响后续 hunk。测试夹具可以裁剪内容，但必须把每段上下文放回锁定上游文件的真实行号，并锁定 hunk header。

回归夹具中的 old-side 内容必须独立声明或直接复制自锁定 preimage，禁止从待验证补丁的 context / 删除行反向生成夹具。否则补丁中错误的空行、缩进或相邻语句会被夹具原样复制，形成“补丁验证自身”的假阳性。夹具还必须包含每个 hunk 前后紧邻的真实源码行；不能在函数开头、闭合括号后或相邻 query 之间自行补空行。
